...
def normalization(planes, norm='gn'):  # set up normalization methods
    if norm == 'bn':
...
class InitConv(nn.Module):
    def __init__(self, in_channels=4, out_channels=16, dropout=0.2):  # initalize convolution network
        super(InitConv, self).__init__()

        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=1)
        self.dropout = dropout

    def forward(self, x):  # create feed forward network with dropout
        y = self.conv(x)
...
class EnBlock(nn.Module):  # encoding block with feed forward network
...
class EnDown(nn.Module):  # create an encode downsample module
...
class Unet(nn.Module):
    ...
    def forward(self, x):  # feed forward network that returns the encoding maps for the skip connections
...
if __name__ == '__main__':  # run the network with no_grad to turn gradient calculations off when detached from the gpu
    # returns the UNET model to generate encoding masks.
    with torch.no_grad():
        import os
